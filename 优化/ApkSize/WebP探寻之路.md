...
那么 WebP 的采样区块会是多少？我们在其他因素保持不变的前提下改变图片规格，选取了 200x200 附近多个规格值，得到了一些数据。将数据可视化之后可以看到凡是以 16x16 倍数（160x160、176x176、192x192、256x256）为规格的图片，有损压缩的比例都明显大于以 4x4 或 8x8 的倍数为规格的图片。
